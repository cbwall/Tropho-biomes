...
library(ggridges)
library(ggplot2)
ggplot(sizing, aes(x = size..mm, y = Inoc.treatment, fill = Inoc.plank.nutr.source)) +
  geom_density_ridges() +
  theme_ridges() +
...
### size by groups
ggplot(sizing, aes(y=size..mm, x=Inoc.treatment, fill=Inoc.plank.nutr.source)) + 
  geom_boxplot(outlier.shape = NA, alpha=0.8)+
  geom_point(pch = 21, position = position_jitterdodge(), alpha=0.6) +
  xlab("Inoculation Water Treatment") +
  ylab("size (mm)")+
...
ggplot(df, aes(x=Inoc.treatment, y=egg.number, group=Inoc.plank.nutr.source, 
